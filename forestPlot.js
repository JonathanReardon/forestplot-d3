async function createForestPlot(
  datafile,
  id = "#forestPlot",
  ...
  const svg = d3
    ...
    ); // Decrease the y translation value

  // Create x and y scales
  const x = d3
    .scaleLinear()
    .range([margin.left - 120, width - margin.right - 80]);
  ...
  if (backgStyle === "style1") {
    ...
    // Set the domain for x and y scales
    x.domain([
      d3.min(data, (d) => +d.lower) - 0.5,
      d3.max(data, (d) => +d.upper) + 0.5,
    ]);
    y.domain(data.map((d) => d.group));
  }
  ...
  svg.selectAll(".domain").style("stroke", xAxisLineColor);

  // Change the color of the x-axis tick labels
  svg.selectAll(".tick text").style("fill", xAxisLabelColor);

  if (backgroundDashlines) {
    svg.selectAll(".x.axis .tick").each(function (d) {
      if (d !== 0) {
        d3.select(this)
          .append("line")
  ...
  svg.select(".y-axis path").attr("stroke-width", 0); // Set the stroke-width of the y-axis path to 0
  ...
  svg
    ...
    .on("mouseover", function (event, d) {
      ...
      const groupAbstract = groupData.abstract
        ? groupData.abstract
      ...
      d3.select("#tooltip_author")
        ...
        .html(
          "<span style='color: #24343c; font-weight: bold;'><b>" +
            groupTitle +
  ...
  lowerLines.transition().duration(duration).delay(1300).style("opacity", 1);

  // Add small vertical lines at the rightmost edge of the "upper" line
  const upperLines = svg
    .selectAll(".upperLine")
    .data(data)
    .enter()
    .append("line")
  ...
  upperLines.transition().duration(duration).delay(1300).style("opacity", 1);
  ...
  pointsGroup
    ...
    .style("fill", dotColor)
    .on("mouseover", function (event, d) {
      d3.select(this).attr("r", 14).style("fill", pointHoverColor);
    ...
    .on("mouseout", function (d) {
      d3.select(this).attr("r", 5).style("fill", dotColor);
  ...
  svg
    ...
    .on("mouseout", function () {
      d3.select(this).attr("stroke", "#5A5A5A"); // Reset the fill color on mouseout

      tooltipDiamond.style("display", "none");
    });

  const tooltipDiamond = d3
    .select("body")
    .append("div")
    .attr("class", "tooltip_diamond")
    .style("display", "none");
